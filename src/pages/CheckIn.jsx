...
//   const [customBodyPart, setCustomBodyPart] = useState("");
//   const [showModal, setShowModal] = useState(false);
//   const [prevMuscleGroup, setPrevMuscleGroup] = useState("");
...
//       setPrevMuscleGroup(muscleGroup);
//     };
...
function CheckIn() {
  ...
  // Update DB if muscle group changes
  useEffect(() => {
    const updateMuscleGroup = async () => {
      if (!user || !userInGym) return;
      ...
      if (selectedBodyPart === "Other" && customBodyPart.trim()) {
        muscleGroup = customBodyPart.trim();
      } else if (
        ["Chest", "Back", "Legs", "Arms", "Shoulders", "Core"].includes(
          selectedBodyPart
      ...
      await set(ref(db, `check-ins/${user.uid}`), data);
      await set(ref(db, `check-ins-history/${Date.now()}-${user.uid}`), data);
  ...
  const handleToggleChange = async (e) => {
    ...
    if (checked) {
      setPrevMuscleGroup(""); // force DB update via useEffect
    } else {
      await remove(ref(db, `check-ins/${user.uid}`));
      setSelectedBodyPart("");
      setCustomBodyPart("");
